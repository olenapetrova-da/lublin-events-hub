-- S2-01A Step D patch: Stage-2 automatic retention (prune outside window)
-- Adds:
--  - public.s2_01a_apply(start, end, prune_outside boolean)  [core]
-- Replaces:
--  - public.s2_01a_apply(start, end) -> wrapper that calls core with prune_outside=true
...
-- allow re-run of THIS migration file without “function exists” errors
DROP FUNCTION IF EXISTS public.s2_01a_apply(date, date, boolean);
...
BEGIN
  ...
  -- Cascades should remove related rows (listings, listing_categories, showtimes, tags).
  ...
  JOIN public.s1_showtimes st ON st.event_id = e.event_id
  JOIN LATERAL generate_series(st.date, st._end_date, interval '1 day') gs ON true
  WHERE gs::date BETWEEN p_start AND p_end;

  -- B) Canonical occurrences
  ...
  -- C) Listings per occurrence per (source,url)
  ...
  FROM tmp_rows r
  JOIN tmp_occ o ON o.title_norm = r.title_norm AND o.date = r.occ_date
  ...
  -- E) Prune stale canonical events INSIDE the window
  ...
  SELECT DISTINCT
    ...
    a.tag_id
  FROM tmp_listings tl
  JOIN tmp_cats c ON c.listing_id = tl.listing_id AND c.source = tl.source
  JOIN public.tag_alias a ON a.source = c.source AND a.raw_value = c.raw_value AND a.enabled = true
  ...
  LEFT JOIN public.tag_alias a ON a.source = c.source AND a.raw_value = c.raw_value AND a.enabled = true
...
-- Wrapper: keep the original 2-arg call, but now it prunes automatically.
