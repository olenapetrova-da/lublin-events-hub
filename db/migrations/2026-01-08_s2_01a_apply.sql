-- S2-01A Step D: Canonicalization routine (SQL)
-- Turns Stage 1 (s1_events/s1_showtimes) into Stage 2 canonical tables.
...
-- ---------------------------------------------------------------------
-- 0) Small schema fix required for multi-day expansion:
-- one source URL can legitimately map to multiple canonical occurrences (different dates),
-- so event_listings must allow repeated (source,url) across different event_id.
-- ---------------------------------------------------------------------
ALTER TABLE public.event_listings
  DROP CONSTRAINT IF EXISTS event_listings_uq_source_url;
...
-- Normalize title for identity merge_key (simple + deterministic)
CREATE OR REPLACE FUNCTION public.leh_normalize_title(p_title text)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
...
-- Deterministic sha1 hex (pgcrypto must exist; you already have it)
CREATE OR REPLACE FUNCTION public.leh_sha1_hex(p_text text)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
...
-- Source priority for "winner" tie-break
CREATE OR REPLACE FUNCTION public.leh_source_priority(p_source text)
RETURNS int
LANGUAGE sql
IMMUTABLE
AS $$
