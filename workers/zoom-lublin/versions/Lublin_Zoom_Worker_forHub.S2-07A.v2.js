...
function parseZoomList(raw, debugSlug = "") {
  ...
  // NOTE: don't anchor on `<div class="..."` because attribute order / newlines can differ.
  // We look for the class token anywhere and then rewind to the surrounding `<div ...>`.
  const WRAP = 'event-card-wrapper';
  const CARD = '<div class="event-card"';

  // Prefer wrapper parsing to read data-infos-ids="..."
  if (html.includes(WRAP)) {
    let i = 0;
    while (true) {
      const hit = html.indexOf(WRAP, i);
      if (hit < 0) break;

      // Rewind to the opening `<div ...>` that contains the wrapper class token.
      const start = html.lastIndexOf('<div', hit);

      // Find the NEXT wrapper token, then rewind to its `<div ...>` so we slice whole blocks.
      const nextHit = html.indexOf(WRAP, hit + WRAP.length);
      const nextStart = nextHit > 0 ? html.lastIndexOf('<div', nextHit) : -1;

      const block = html.slice(start >= 0 ? start : hit, nextStart > 0 ? nextStart : html.length);
      i = nextStart > 0 ? nextStart : html.length;

      // Wrapper open tag contains data-infos-ids
      const openEnd = block.indexOf(">");
      const openTag = openEnd > 0 ? block.slice(0, openEnd + 1) : "";

      // Robust extraction: tolerates spaces + single/double quotes
      const infosRaw =
      ...
      out.push({
        ...
        Category: category || "",
        Link: href,
        event_ref: evref,   // used in n8n and for debug_slug matching
        "Payment for Entry": isFree ? "No" : "",
        Source: "zoom.lublin.pl",
        _EndDate: End || Date || "",  // ADR-0009: ensure present; default to Date
        _fp_url: urlPath(href),

        // Debug fields appear ONLY when debugSlug matches this event_ref
        ...(debug ? {
  ...
  while (true) {
    ...
    out.push({
      ...
      Category: category || "",
      Link: href,
      event_ref: evref,   // -- added to Official adapter to address issues with multiple pages for the same event. Here is for consistant use in n8n
      "Payment for Entry": isFree ? "No" : "",
      Source: "zoom.lublin.pl",
      _EndDate: End || Date || "",
      _fp_url: urlPath(href),
